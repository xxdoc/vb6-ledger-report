...
Begin VB.Form frmAddEditPromotionPay 
   BackColor       =   &H80000000&
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin prjLedgerReport.uctlDate uctlProDate 
         ...
         _ExtentY        =   661
      End
      Begin prjLedgerReport.uctlTextBox txtProSaleCode 
         Height          =   450
         Left            =   2400
         TabIndex        =   11
         Top             =   1800
         ...
         _ExtentY        =   794
      End
      Begin prjLedgerReport.uctlTextBox txtProSaleName 
         Height          =   450
         Left            =   3900
         TabIndex        =   12
         Top             =   1800
   ...
   End
End
Attribute VB_Name = "frmAddEditPromotionPay"
...
Private ItemPromotion  As CPromotionPayCustom
...
Public SaleProColl As Collection
...
Dim ItemCount As Long
   ...
   If Flag Then
      ...
      ItemPromotion.PRO_ID = ID
      ItemPromotion.QueryFlag = 1
      If Not glbDaily.QueryPromotionPayCustomer(ItemPromotion, m_Rs, ItemCount, IsOK, glbErrorLog) Then
   ...
   If ItemCount > 0 Then
      ...
      uctlProDate.ShowDate = ItemPromotion.PRO_DATE
      txtProSaleCode.Text = ItemPromotion.SALECODE_PRO
      txtProSaleName.Text = ItemPromotion.SALENAME_PRO
      txtProCustomerCode.Text = ItemPromotion.CUSTOMERCODE_PRO
      txtProCustomerName.Text = ItemPromotion.CUSTOMERNAME_PRO
      txtProSTKCOD.Text = ItemPromotion.STKCOD_PRO
      txtProSTKName.Text = ItemPromotion.STKNAME_PRO
      txtProAmount.Text = ItemPromotion.AMOUNT_PRO
...
Dim IsOK As Boolean
   ...
   If Not VerifyDate(lblProSaleCode, uctlProDate, False) Then
      Exit Function
   End If
   If Not VerifyTextControl(lblProSaleCode, txtProSaleCode, False) Then
      Exit Function
   End If
   If Not VerifyTextControl(lblProSaleCode, txtProSaleName, False) Then
      Exit Function
   End If
   ...
   ItemPromotion.PRO_DATE = Format(uctlProDate.ShowDate, "DD") & "/" & Format(uctlProDate.ShowDate, "MM") & "/" & Format(uctlProDate.ShowDate, "YYYY")
   ItemPromotion.SALECODE_PRO = txtProSaleCode.Text
   ItemPromotion.SALENAME_PRO = txtProSaleName.Text
   ItemPromotion.CUSTOMERCODE_PRO = txtProCustomerCode.Text
   ItemPromotion.CUSTOMERNAME_PRO = txtProCustomerName.Text
   ItemPromotion.STKCOD_PRO = txtProSTKCOD.Text
   ItemPromotion.STKNAME_PRO = txtProSTKName.Text
   ItemPromotion.AMOUNT_PRO = Val(txtProAmount.Text)
   ItemPromotion.YYYY_MM = Year(uctlProDate.ShowDate) & "-" & Format(Month(uctlProDate.ShowDate), "00")
   
   Call EnableForm(Me, False)
   If Not glbDaily.AddEditPromotionPayCustomer(ItemPromotion, IsOK, True, glbErrorLog) Then
      glbErrorLog.ShowErrorLog (LOG_FILE_MSGBOX)
      SaveData = False
      Call EnableForm(Me, True)
      Exit Function
...
Private Sub InitFormLayout()
   ...
   Call InitNormalLabel(lblProSaleCode, MapText("พนักงานขาย"))
   ...
   Call txtProSaleCode.SetTextLenType(TEXT_STRING, glbSetting.CODE_TYPE)
   Call txtProSaleName.SetTextLenType(TEXT_STRING, glbSetting.NAME_TYPE)
...
Private Sub Form_Load()
   ...
   Set ItemPromotion = New CPromotionPayCustom

   Set SaleProColl = New Collection
...
Private Sub txtProSaleCode_Change()
Dim TempSale As COESLM
   m_HasModify = True
   Set TempSale = GetObject("COESLM", SaleProColl, txtProSaleCode.Text, False)
   If Not TempSale Is Nothing Then
      txtProSaleName.Text = TempSale.SLMNAM
   Else
      txtProSaleName.Text = ""
   End If
...
Private Sub txtProSaleName_Change()
   m_HasModify = True
End Sub
