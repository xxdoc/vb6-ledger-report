...
Begin VB.Form frmCommissionChart 
   ClientHeight    =   8520
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11910
   Icon            =   "frmCommissionChart.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   Moveable        =   0   'False
   ScaleHeight     =   8520
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin MSComctlLib.ImageList ImageList1 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmCommissionChart.frx":08CA
               Key             =   ""
            EndProperty
            BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmCommissionChart.frx":11A4
               Key             =   ""
            EndProperty
         EndProperty
      End
      ...
      Begin Threed.SSCommand cmdOK 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmCommissionChart.frx":1A7E
         ButtonStyle     =   3
      End
      ...
      Begin Threed.SSCommand cmdDelete 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmCommissionChart.frx":1D98
         ButtonStyle     =   3
      End
      Begin Threed.SSCommand cmdAdd 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmCommissionChart.frx":20B2
         ButtonStyle     =   3
      End
   ...
   End
End
Attribute VB_Name = "frmCommissionChart"
...
Private Const MODULE_NAME = "frmCommissionChart"
...
Private Function GetCommissionChart(TempID As Long, TempCol As Collection) As CCommissionChart
Dim L As CCommissionChart

   For Each L In TempCol
      If L.COMMISSION_CHART_ID = TempID Then
         Set GetCommissionChart = L
         Exit Function
      End If
   Next L
End Function
...
'      glbErrorLog.ShowUserError
   ...
   Set L = GetObject("CCommissionChart", m_Commissions, Trim(Str(ID)))
   
   glbErrorLog.LocalErrorMsg = "ต้องการลบข้อมูล รหัสพนักงานขาย " & L.SALE_ID & " ใช่หรือไม่ ?"
   If glbErrorLog.AskMessage = vbNo Then
      Exit Sub
   End If
   
   L.COMMISSION_CHART_ID = ID
   
   If Not glbDaily.DeleteCommissionChart(L, IsOK, True, glbErrorLog) Then
   ...
   Call LoadCommissionChart(Nothing, m_Commissions, m_MasterFromTo.MASTER_FROMTO_ID)
...
Dim ItemCount As Long
   ...
   If m_HasModify Or m_MasterFromTo.MASTER_FROMTO_ID <= 0 Then
   ...
   frmAddEditCommissionChart.HeaderText = "เพิ่มข้อมูลแผนภูมิการคิดคอมมิตชั่น"
   frmAddEditCommissionChart.ShowMode = SHOW_ADD
   frmAddEditCommissionChart.FK_ID = m_MasterFromTo.MASTER_FROMTO_ID
   Load frmAddEditCommissionChart
   frmAddEditCommissionChart.Show 1

   If frmAddEditCommissionChart.OKClick Then
      Call EnableForm(Me, False)
      Call LoadCommissionChart(Nothing, m_Commissions, m_MasterFromTo.MASTER_FROMTO_ID)
      Call InitMainTreeview("", m_Commissions)
      Call EnableForm(Me, True)
   ...
   Unload frmAddEditCommissionChart
   Set frmAddEditCommissionChart = Nothing
...
Dim TableName As String
   ...
   Call EnableForm(Me, False)
   frmAddEditCommissionChart.HeaderText = "แก้ไขข้อมูลแผนภูมิการคิดคอมมิตชั่น"
   frmAddEditCommissionChart.ShowMode = SHOW_EDIT
   frmAddEditCommissionChart.ID = ID
   frmAddEditCommissionChart.FK_ID = m_MasterFromTo.MASTER_FROMTO_ID
   Load frmAddEditCommissionChart
   frmAddEditCommissionChart.Show 1
...
'   OKClick = False
   Unload Me
End Sub
Private Function GetIconNo(O As CCommissionChart) As Long
...
Dim L As Long
   ...
   For Each O In TempColl
     If O.PARENT_ID = PID Then
           Set S = GetSlm(m_SaleName, O.SALE_ID, False)
           ...
           End If
         If Level = 0 Then
            Set Node = trvMain.Nodes.Add(, tvwFirst, NodeID & O.COMMISSION_CHART_ID, "[" & O.COMMISSION_CHART_ID & "]  " & saleName & " __ เขต : " & O.AREA_NAME & " __ ประเภทสินค้า : " & O.GOODS_GROUP_NAME & " __งบประมาณ " & FormatNumber(O.BUDGET), GetIconNo(O))        '& " " & O.GetFieldValue("RETURN_NAME")
            Node.Tag = O.COMMISSION_CHART_ID
            Call GenerateTree(TempColl, Node, NodeID & O.COMMISSION_CHART_ID, O.COMMISSION_CHART_ID, Level + 1)
            O.CHILD_COUNT = Level
         Else
            NewNodeID = NodeID & "-" & O.COMMISSION_CHART_ID
          Set Node = trvMain.Nodes.Add(N, tvwChild, NewNodeID, "[" & O.COMMISSION_CHART_ID & "]  " & saleName & " __ เขต : " & O.AREA_NAME & " __ ประเภทสินค้า : " & O.GOODS_GROUP_NAME & " __งบประมาณ " & FormatNumber(O.BUDGET), GetIconNo(O))      '& " " & O.GetFieldValue("RETURN_NAME")
            Node.Tag = O.COMMISSION_CHART_ID
           Call GenerateTree(TempColl, Node, NewNodeID, O.COMMISSION_CHART_ID, Level + 1)
             O.CHILD_COUNT = Level
         End If
         Node.Expanded = True
     End If
...
Private Sub InitMainTreeview(Caption As String, TempColl As Collection)
   ...
   Call GenerateTree(TempColl, Nothing, "ROOT", 0, 0)
End Sub
